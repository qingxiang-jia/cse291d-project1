 ...
 * <p>
 * <p>
 * This class contains fields and convenience methods related to the result of running a single
 * test. It is used internally by <code>Series</code> to pass test results. For failed tests, it is
 * returned in <code>SeriesReport</code> objects.
 */
class TestReport implements Serializable {
  ...
  /**
   * Reason for test failure, or <code>null</code> if the test was successful.
   */
  private final Throwable stop_cause;
  /**
   * Task message when the test failed, or <code>null</code> if there was no task message, or the
   * test was successful.
   */
  private final String task;
  /**
   * Reason for test cleanup failure, or <code>null</code> if the test was cleaned up successfully.
   */
   ...
   * <p>
   * <p>
   * If <code>stop_cause</code> is wrapped in a <code>FailedDuringTask</code> object, it is
   * unwrapped, and the task noted in the <code>task</code> field.
   */
  ...
  /**
   * Returns <code>true</code> if and only if the test was successful: if both the test stop cause
   * and the cleanup stop cause are <code>null</code>.
   */
  boolean successful() {
  ...
  /**
   * Returns <code>true</code> if and only if the test stopped with a fatal error, or cleanup
   * stopped with any error (all cleanup erros are fatal).
   */
  boolean fatal() {
   ...
   * <p>
   * <p>
   * The test name is printed. If the main test failed, then the reason for the failure is printed.
   * The task being performed during the failure is shown, if there was one. If the failure was not
   * a timeout, the stack trace is shown as well. If the cleanup task failed, a description of the
